...
#undef F7LY_PERF_DESC
        ...
        bool arch_pmu_probe_event(ProfileEvent event)
        {
            ...
            const uint64 old_sie = r_sie();
            w_sie(old_sie | k_sie_lcofie);
            const bool overflow_interrupt = (r_sie() & k_sie_lcofie) != 0;
            w_sie(old_sie);
    ...
    uint64 timebase_hz() { return tmm::clock_frequency_hz(); }
