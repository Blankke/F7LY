// 板级网卡到 ONPS 的适配层，实现不依赖具体 VirtIO/GMAC 设备。

#include "onps_adapter.hh"
...
namespace net
{
    ...
    bool adapter_init()
    {
        ...
        // ONPS 当前没有 DHCPv4 客户端，因此读取平台提供的网络层默认值。
        // 配置接口独立于网卡设备，避免 GMAC/VirtIO 驱动承担 IP 策略。
        const NetworkConfig &config = default_network_config();
        ipv4_config.unAddr = inet_addr(config.ipv4_address);
        ipv4_config.unSubnetMask = inet_addr(config.ipv4_netmask);
        ipv4_config.unGateway = inet_addr(config.ipv4_gateway);
        ipv4_config.unPrimaryDNS = inet_addr(config.ipv4_dns);
        ipv4_config.unBroadcast = inet_addr(config.ipv4_broadcast);
