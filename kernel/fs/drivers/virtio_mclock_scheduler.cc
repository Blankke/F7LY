...
namespace virtio_blk
{
    ...
    MClockScheduler::DispatchDecision MClockScheduler::dequeue_single_active_class(uint64 now_us)
    {
        ClassState *active_class = nullptr;
        for (int i = 0; i < k_class_count; ++i)
        {
            if (class_state_[i].flow_head == nullptr)
            {
                continue;
            }
        ...
        FlowState *selected_flow =
            active_class->rr_hint != nullptr ? active_class->rr_hint : active_class->flow_head;
        if (selected_flow == nullptr || selected_flow->head == nullptr)
        {
            return {nullptr, 0};
        }

        IoRequest *selected_request = selected_flow->head;
        if (selected_request->l_tag_us > now_us)
        {
            return {nullptr, selected_request->l_tag_us};
        }

        IoRequest *queued = pop_flow_head(selected_flow);
        if (queued != selected_request)
        {
            panic("MClockScheduler::dequeue_single_active_class: queue mismatch");
        }
    ...
    {
        DispatchDecision single_class_decision = dequeue_single_active_class(now_us);
        if (single_class_decision.request != nullptr || single_class_decision.next_gate_us != 0)
        {
            return single_class_decision;
        }
