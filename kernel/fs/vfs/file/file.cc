        ...
        BsdFlockEntry *alloc_flock_entry(const eastl::string &path)
        {
	                                                    ...
	                                                    RestoreFn restore_identity)
	        {
	            ...
	            for (int i = 0; i < entry_count; ++i)
	            {
	                ...
	                {
	                    preserved.push_back(make_record_lock_slice(original, entry_start, request_start));
	                }
	                if (request_end < entry_end)
	                {
	                    preserved.push_back(make_record_lock_slice(original, request_end, entry_end));
	                }
	            }
