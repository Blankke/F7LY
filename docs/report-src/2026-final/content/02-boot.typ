...
RISC-V 和 LoongArch 仍然分别保留自己的 `entry.S`、早期栈和寄存器初始化代码，但启动代码不直接初始化文件系统、网络或调度器，而是先准备本次启动所需的基本信息：
...
本阶段明确了不同信息的来源：CPU 的寄存器机制由架构层处理，板上固定的 UART/IRQ/MMIO 资源由平台配置提供，本次启动发现的 CPU 与 RAM 数量由 DTB 提供，设备运行状态由具体驱动维护。公共 `memlayout` 不保存板级设备地址，也不在 DTB 解析失败后猜测另一块机器的内存上限。
