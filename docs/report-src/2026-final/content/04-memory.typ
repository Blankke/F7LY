...
fork 时父子地址空间共享只读物理页并增加引用计数，写访问触发写时复制缺页。若页面来自文件对象，私有映射的写入会在当前进程的 VMA 中记录为独立页面，不写回原文件；共享映射则继续由同一个 `VmObject` 提供页面。只有在引用计数和映射属性允许时，缺页路径才可以原地升级页面，否则分配新页并复制内容。
