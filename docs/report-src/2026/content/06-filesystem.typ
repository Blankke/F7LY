...
调度器通过 EWMA 动态估计磁盘有效吞吐（初始 64 MB/s，每次完成时以 $alpha = 1/8$ 更新），使 W-tag 的计算自适应硬件实际能力。当仅有一个类活跃时，调度器走快速路径直接轮转，避免跨类扫描开销。F7LY 的 I/O 调度方案经历了 多次迭代，最终选择 mClock 是因为其 R/W/L 三维控制在保底公平与权重分享之间取得了较好的平衡，且数据结构简单、无堆排序开销。
...
与挂载系统配合的是 mount namespace 机制。每个进程属于一个 mount namespace，拥有独立的挂载点视图——在一个 namespace 中执行的挂载或卸载不会影响其他 namespace。F7LY 通过 `vfs_clone_mount_namespace` 创建新的 namespace（通常配合 `clone` 创建容器时使用），通过引用计数（`vfs_hold_mount_namespace` / `vfs_put_mount_namespace`）管理 namespace 的生命周期，namespace 内最后一份挂载视图被释放后整个 namespace 自动销毁。

路径解析是每次文件访问的第一步，负责将用户态传入的路径字符串转换为 VFS 可操作的 inode。`normalize_path` 先对路径做语法规范化，然后 `vfs_resolve_path` 完成语义解析——区分虚拟路径、设备路径和磁盘路径，将最终找到的 inode 返回给调用方。
== 6.8 虚拟文件与特殊设备
