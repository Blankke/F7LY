...
2026 年的 F7LY-OS 已经从“能够启动并跑通基础测例”的教学内核，推进到面向 Linux ABI、双架构连续测例和复杂用户程序的系统原型。内核主体仍保持 C++23 freestanding 的实现方式，但系统边界已经明显扩大：RISC-V 与 LoongArch 共享同一套内核服务，用户态同时覆盖 musl 与 glibc 动态链接程序，根文件系统以 ext4 为主，辅以 FAT32 数据盘、虚拟文件系统、socket、epoll、eventfd、memfd、futex、信号、POSIX timer 这些现代 Linux 程序常用接口。
== 已验证的功能

=== 连续测例通过情况

截至当前 scoreboard 小分文件记录，F7LY-OS 在四组合评测口径下共登记 12892 条组合测例。这里的“组合测例”按架构和 C 运行库分别计数，即同一个测例在 `RISC-V + musl`、`RISC-V + glibc`、`LoongArch + musl`、`LoongArch + glibc` 下会被视为不同验证项。

当前连续测例覆盖以下类型：

- *basic*：四组合共 128 条组合测例，覆盖基础进程、文件、目录、时间、内存映射、管道、挂载与卸载接口。
- *busybox*：四组合共 220 条组合测例，覆盖 BusyBox applet、shell 脚本、常用命令、文件浏览和命令行环境初始化。
- *cyclictest*：四组合共 20 条组合测例，覆盖实时性和定时器延迟观测；当前 scoreboard 尚未记录通过状态。
- *iozone*：四组合共 32 条组合测例，覆盖顺序读写、随机读、反向读、跨步读、fwrite/fread、pwrite/pread、pwritev/preadv。
- *iperf*：四组合共 32 条组合测例，覆盖 TCP/UDP 吞吐、socket 创建、连接、读写、关闭和脚本化网络测试；当前 scoreboard 尚未记录通过状态。
- *libcbench*：四组合共 4 条组合测例，覆盖 C 运行库性能路径、线程库、内存分配和常用 libc 接口。
- *libctest-static*：两组合共 428 条组合测例，覆盖静态链接入口下的 libc 行为、pthread、TLS、数学函数、stdio、文件接口和进程接口。
- *libctest-dynamic*：两组合共 440 条组合测例，覆盖动态链接入口下的 libc 行为、动态装载器、共享库解析、pthread、TLS、stdio、文件接口和进程接口。
- *lmbench*：四组合共 96 条组合测例，覆盖系统调用延迟、上下文切换、管道、文件、内存、进程创建和通信微基准；
- *LTP*：四组合共 11320 条组合测例，是当前最大规模的 Linux ABI 连续测例集合；scoreboard 已记录 853 条单测例通过记录。
- *lua*：四组合共 36 条组合测例，覆盖解释器启动、脚本执行、文件 I/O、随机数、字符串、排序和数学函数。
- *netperf*：四组合共 24 条组合测例，覆盖 TCP_STREAM、TCP_RR、UDP_STREAM、UDP_RR、socket option 和网络吞吐/延迟脚本；


LTP 已确认通过项覆盖 fork/exec/wait、mmap/munmap/mprotect、open/read/write/stat、signal、futex、timer、socket、epoll 这些核心 Linux ABI 行为。其他测例类型的 scoreboard 记录正在继续维护。

这些结果说明当前内核已经具备运行真实用户态连续测例套件的基础稳定性：测例不再只验证单个 syscall 的存在，而是在动态链接器、文件系统、地址空间、线程同步、信号恢复和进程生命周期之间进行组合压力测试。

=== 用户程序运行能力

除自动连续测例外，F7LY-OS 已经能够支撑 5 类代表性复杂用户程序的关键运行路径：shell、gcc、rustc、vim 和 git。这些程序覆盖的不是单一接口，而是现代 Unix 用户态环境的典型组合需求：命令解析与进程派生、阶段化编译、终端控制、多线程同步、内存映射、文件系统元数据、仓库对象管理以及 socket 网络通信。
...
整体来看，F7LY-OS 2026 年的核心成果是把若干原本分散的内核能力收束成一套面向 Linux 用户态程序的运行环境。进程、线程、地址空间、文件系统、网络和系统调用不再是彼此孤立的模块，而是围绕真实程序的启动、运行、同步、I/O 和退出路径协同工作。尤其是动态 ELF 装载、musl/glibc loader 路径重写、shebang、VMA / VmObject、futex、epoll、socket_file 和 VFS `file` 多态体系，使内核可以用更接近 Linux 的方式承接复杂应用。

后续工作仍应围绕“减少特殊分支、提高长连续测例稳定性、扩大真实应用覆盖面”展开。首先，LTP 中尚未通过或暂未确认的项目需要继续按 syscall 族和子系统聚类推进，避免只修单点而破坏其他组合。其次，glibc 侧 libctest 和大型工具链应用对 ABI 细节更敏感，仍需要继续校准 `statx`、termios、socket option、资源限制、命名空间视图和 `/proc` / `/sys` 虚拟文件内容。最后，git 网络、vim 全屏交互、gcc/rustc 完整编译链这些真实工作流应逐步从“能启动、能覆盖关键路径”推进到“能稳定完成端到端任务”，让 F7LY-OS 的评测能力进一步转化为真实用户态生态能力。
