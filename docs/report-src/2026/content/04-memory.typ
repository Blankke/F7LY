...
=== 4.2.2 进程内存管理器（ProcessMemoryManager）
...
- *VMA 描述符（`VmArea`）*：记录一段地址区间的元数据——起止地址、权限、标志位、所属类型（ELF 段 / 堆 / 栈 / mmap / SysV SHM）、增长策略，以及指向后端对象的指针。它自身不包含任何物理页分配逻辑。
- *容器与索引（`VMASpace` $+$ `VmaMapleTree`）*：`VMASpace` 是 VMA 的运行时容器，内部用 `eastl::list` 链表持有所有 `VmArea` 对象，同时用一棵 B+Tree 风格的 `VmaMapleTree` 按地址建立索引。插入、删除、查找均为 O(log N)，叶子节点带有链表指针以支持快速顺序遍历和空闲区间搜索。
- *后端对象（`VmObject`）*：将“数据从哪里来”抽象为独立对象。匿名映射对应 `AnonVmObject`（缺页时分配零页），文件映射对应 `FileVmObject`（缺页时读文件），SysV 共享内存对应 `SysvShmVmObject`。三者共享统一的 `prepare_page()` 接口，缺页处理路径不再需要区分后端类型。

三层协作的典型流程为：缺页异常发生时，`ProcessMemoryManager::fault_page()` 通过 `VMASpace` 查找覆盖该地址的 `VmArea`，再调用其 `VmObject::prepare_page()` 获取物理页，最后安装到页表中。下面分别展开三层。

==== 4.2.3.1 VMA 描述符（VmArea）
...
==== 4.2.3.2 VMASpace 与 Maple Tree 索引
...
`VmaMapleTree` 的设计参考了 Linux 的 Maple Tree，但做了大幅简化。它是一棵纯索引树——树节点只存 VMA 的指针和排序键（`addr`），不拥有数据的所有权。叶子节点最多容纳 12 个 VMA 指针，内部节点最多持有 12 个子节点引用，足够的宽度使树高在 256 个 VMA 时不超过三层。叶子节点之间通过双向链表连接，使得给定一个地址，找出覆盖它的 VMA只需 O(log N) 从根走到叶，而从当前位置向后遍历所有 VMA则直接沿叶节点链表顺序走，不需要回溯。

快速区间索引对 `mmap` 同样关键。`mmap` 需要在两个已有 VMA 之间找到足够大的空隙。`VmaMapleTree` 沿叶子链表顺序扫描相邻 VMA 之间的间隔，在 O(有效 VMA 数量) 时间内给出第一个满足对齐和大小要求的空闲区间。相比线性扫描需要遍历满 256 个槽位再检查间隔，树索引只需遍历实际使用的 VMA，槽位越少越快。
...
==== 4.2.3.3 VMObject 后端抽象

旧架构中，匿名页的零填充、文件页的磁盘读取、共享内存页的段内查找，三套逻辑分散在 mmap 和缺页处理的不同分支里。`VmObject` 的引入将这些数据来源的差异统一到一个抽象基类背后，对外只暴露一个接口：给定 VMA 区间内的某个页序号和访问类型（读/写/执行），返回一个可安装到页表中的物理页。
