...
- [x] 2026-08-15 LA 内层 QEMU ENOMEM 与本轮性能优化完成待验收：评测附件中的 Rust 冷编译已成功，实际失败点是随后内层 QEMU 预留 8 GiB guest RAM；根因是单个 VMA 长度经 32 位 `int` 截断。VMA 长度和文件偏移现统一为 64 位，并补齐大映射拆分、合并、扩展和重映射链路；同一 LA 基线内核的 8 GiB 窄测稳定在 `mmap errno=12`，修复后 RV/LA 均连续两轮完成预留、首尾触页、中段 `mprotect` 与整段 `munmap`。Buddy 空闲页统计改为分配/释放时 O(1) 记账，LA 在 32 MiB 碎片化后执行 5000 次 `sysinfo` 的单次耗时由 215120 ns 降至 12553 ns（17.1 倍）；匿名写缺页及完整页 COW/文件覆盖不再重复清零。两架构 CAgent 均 10/10，双架构构建通过；证据为 `logs/run/output_l_20260815-234245_buildstorm-perf.txt`、`logs/run/output_{l_20260815-232940,r_20260815-233041}_buildstorm-perf.txt`、`logs/run/output_{l_20260815-233152,r_20260815-233249}_buildstorm-perf.txt` 和 `logs/run/output_{l_20260815-234038,l_20260815-233813}_buildstorm-perf.txt`。完整官方 Docker 并发 workflow 仍由用户决定何时执行。
- [x] 2026-08-16 RV BuildStorm/CAgent 回归防护完成待验收：VMA 的长度和文件偏移虽然已扩为 64 位，旧式公共缺页后备路径仍把“文件基准偏移 + VMA 页偏移”收窄成 `int`；该链路现全程使用 64 位并显式检查加法与底层 `long` 边界，避免大偏移文件页读错位置。mmap 与 fork 上层无条件调用统一的用户范围准备入口，只有 LA 在入口内部保留 TLB refill 必需的页表骨架预建。RV/LA 的 3 GiB 文件偏移 mmap+fork 专项和 CAgent 10/10 均通过；8 GiB RAM 预留两轮均通过，LA 约 3.20/3.24 秒，与修复前约 3.15/3.22 秒同档。RV 正式 BuildStorm 前段已越过评测日志中的 `prebuild-*.sh -> mkdir` 失败点并进入 146 个目标产物的 crate 编译，随后主动停止窄测；由于原失败在本地不能稳定复现，这只能证明当前版本未再出现同类故障，不把该 32 位缺陷表述为动态链接器报错的唯一已证根因。证据为 `logs/run/output_{r_20260816-222542,l_20260816-222702}_buildstorm-perf.txt`、`logs/run/output_{r_20260816-222749,l_20260816-222836}_buildstorm-perf.txt` 和 `logs/run/output_r_20260816-222936_buildstorm-perf.txt`。宿主无法分配 LA 36 GiB QEMU，完整官方双架构并发 workflow 仍由用户决定何时执行。
- [x] 2026-08-18 RV VirtIO 块完成内存序修复完成待验收：评测日志中的负耗时来自 CAgent 前后两次 `date` 之一因并发读取 libc 得到坏 ELF 而启动失败，并非系统时钟倒退；BuildStorm 随后在用户程序已受损时进入 `mkdirat` 并触发内核近空指针异常。根因是 split-ring 完成路径在读取 `used->idx` 之前执行屏障，未建立“观察完成后再读取 used elem/status/DMA 数据”的 acquire 顺序，提交路径也缺少 `avail->idx` 到 notify 的屏障；RV 弱内存序下可把 bounce buffer 旧内容作为成功读返回，而 LA 的强屏障时序不易暴露。公共队列现按 descriptor/slot -> barrier -> avail index -> barrier -> notify 发布，并在 volatile 观察 used index 后执行架构屏障再消费完成数据；status 初值改为非成功值，避免不可见完成被静默接受。RV/LA 构建通过，机器码分别确认 `fence`/`dbar 0`，RV 8 核短 CAgent 回归 10/10；按用户要求未运行本地 BuildStorm 压测，完整评测机双架构并发结果待外部验收。短回归日志为 `logs/run/output_r_20260818-164830_buildstorm-perf.txt`。
