#!/usr/bin/env bash
# 用法示例：
#   tools/patch_loongarch_libctest_llsc.sh images/sdcard-la.img
# 说明：
#   这个脚本会原地修补 LoongArch 镜像里 musl 预置二进制中的坏 ll/sc 指令序列。
#   目前覆盖 libc-test 的 entry-static、musl libc.so，以及 libc-bench。
#   脚本是幂等的：如果镜像已经补过，会直接跳过。
...
# entry-static.exe 的 __vm_lock 热点。
...
# libc-bench 的 b_malloc_thread_stress 线程计数热点。
patch_file_in_image "$IMAGE_PATH" "/musl/libc-bench" "libc-bench-musl" \
    0x0fe1c \
    ac81ea028c0100208c058002ae81ea02cc010021 \
    ae81ea02cc0100208c05800200004003cc010021 \
    "libc-bench::thread_counter"
